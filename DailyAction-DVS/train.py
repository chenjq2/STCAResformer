...
import model, utils
from DailyAction_dvs import DailyAction_DVS
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='DailyAction_DVS', help='dataset')
    parser.add_argument('--num-classes', type=int, default=12, metavar='N',
                        help='number of label classes (default: 1000)')
    parser.add_argument('--data-path', default='/data/cjq/RS_LIF/Datasets/DailyAction_DVS', help='dataset')
    ...
    parser.add_argument('--output-dir', default='/data/cjq/SpikingTransformer/dailyAction/logs/', help='path where to save')
...
def evaluate(model, criterion, data_loader, device, print_freq=100, header='Test:', classes=None):
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    all_preds = []
    all_targets = []
    ...
    with torch.no_grad():
        for image, target in metric_logger.log_every(data_loader, print_freq, header):
            ...
            predictions_proba.extend(probabilities.cpu().numpy())

    cm = confusion_matrix(all_targets, all_preds)
    if classes is not None:
        plot_confusion_matrix(cm, classes)
        plot_roc_curve(predictions_proba, all_targets, classes)
...
def load_data(dataset_dir, distributed, T):
    ...
    # dataset_train = HARDVS(root=dataset_dir, train_test_val='train', data_type='frame', frames_number=T, split_by='number')
    # dataset_test = HARDVS(root=dataset_dir, train_test_val='test', data_type='frame', frames_number=T, split_by='number')

    origin_set = DailyAction_DVS(root=dataset_dir, data_type='frame', frames_number=T, split_by='number')
    dataset_train, dataset_test = split_to_train_test_set(0.8, origin_set, 12)
...
def plot_confusion_matrix(cm, classes):
    ...
    ax = sns.heatmap(cm_normalized, annot=True, vmin=0, vmax=1.0, fmt='.2%', cmap='gray_r', xticklabels=classes, yticklabels=classes, mask=(cm==0), cbar=False)
...
def plot_roc_curve(predictions_proba, targets, classes):
    ...
    for i in range(n_classes):
        fpr[i], tpr[i], _ = roc_curve(y_bin[:, i], predictions_proba[:, i])
        roc_auc[i] = auc(fpr[i], tpr[i])

    # Compute micro-average ROC curve and ROC area
    fpr["micro"], tpr["micro"], _ = roc_curve(y_bin.ravel(), predictions_proba.ravel())
    roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])

    # Plot ROC curve
    plt.figure(figsize=(10, 8))
    lw = 2  # Line width
    colors = cycle(['aqua', 'darkorange', 'cornflowerblue', 'green', 'red', 'purple', 'brown', 'pink', 'gray', 'olive', 'blue', 'gold'])
    markers = cycle(['+', 'o', '*', '.', 'x', 's', 'd', '^', 'v', 'p', '<', 'h'])
    ...
    for i, color, marker in zip(range(n_classes), colors, markers):
        plt.plot(fpr[i], tpr[i], color=color, marker=marker, lw=lw,
...
def main(args):
    ...
    if args.tb and utils.is_main_process():
        ...
        purge_step_test = args.start_epoch
        train_tb_writer = SummaryWriter(output_dir + '_logs/train', purge_step=purge_step_train)
        test_tb_writer = SummaryWriter(output_dir + '_logs/test', purge_step=purge_step_test)
        with open(output_dir + '_logs/args.txt', 'w', encoding='utf-8') as args_txt:
            args_txt.write(str(args))

        print(f'purge_step_train={purge_step_train}, purge_step_test={purge_step_test}')
