...
def parse_args():
    ...
    parser.add_argument('--dataset', default='PAF', help='dataset')
    parser.add_argument('--num-classes', type=int, default=10, metavar='N',
                        help='number of label classes (default: 1000)')
    parser.add_argument('--data-path', default='/data/cjq/RS_LIF/Datasets/ActionRecognition/', help='dataset')
    ...
    parser.add_argument('--output-dir', default='/data/cjq/SpikingTransformer/PAF/logs/', help='path where to save')
...
def load_data(dataset_dir, distributed, T):
    ...
    origin_set = ActionRecognition(root=dataset_dir, data_type='frame', frames_number=T, split_by='number')
    dataset_train, dataset_test = split_to_train_test_set(0.8, origin_set, 10)
...
def plot_confusion_matrix(cm, classes):
    ...
    ax = sns.heatmap(cm_normalized, annot=True, square=True, vmin=0, vmax=1.0, fmt='.2%', cmap='gray_r', xticklabels=classes, yticklabels=classes, mask=(cm==0), cbar=False)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, num_epochs):
        ...
        classes = ["arm crossing", "get up", "jumping", "kicking", "picking up", "sit down", "throwing", "turning around", "walking", "waving"]
        test_loss, test_acc1, test_acc5 = evaluate(model, criterion, data_loader_test, device=device, header='Test:', classes=classes)
        if test_tb_writer is not None:
            if utils.is_main_process():
