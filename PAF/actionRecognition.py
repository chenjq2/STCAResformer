...
def parse_raw_address(addr,
    ...
    # print(f"Raw address: {addr[:10]}")  # 输出前10个原始地址
    ...
    # print(f"Parsed x: {x[:10]}")  # 输出前10个解析后的x坐标
    # print(f"Parsed y: {y[:10]}")  # 输出前10个解析后的y坐标
    # print(f"Parsed polarity: {polarity[:10]}")  # 输出前10个解析后的极性
    return x, y, polarity
...
class ActionRecognition(sjds.NeuromorphicDatasetFolder):
    def __init__(
            self,
            root: str,
            data_type: str = 'event',
 ...
 <https://internal-journal.frontiersin.org/articles/10.3389/fnins.2017.00309/full>`_.
    ...
    def resource_url_md5() -> list:
        ...
        '''
        return [
            ('ActionRecognition.zip', 'https://ndownloader.figshare.com/files/7712839', None)
        ]
    ...
    def load_origin_data(file_name: str) -> Dict:
        ...
        with open(file_name, 'rb') as fp:
            ...
            # print(f"First 50 x values: {x[:50]}")
            # print(f"First 50 y values: {y[:50]}")
            # print(f"First 10 polarity values: {p[:10]}")
            
            # if np.any(x >= 240) or np.any(y >= 180):
            #     print(f"Out-of-bounds x values: {x[x >= 240]}") # x have maximum 240 so maybe is 240 - x
            #     print(f"Out-of-bounds y values: {y[y >= 180]}")
            #     raise ValueError("Data contains out-of-bounds values for x or y coordinates")
            return {'t': t, 'x': 345 - x, 'y': 259 - y, 'p': 1 - p.astype(int)}
    ...
    def read_aedat_save_to_np(bin_file: str, np_file: str):
        events = ActionRecognition.load_origin_data(bin_file)
        np_savez(np_file,
                 t=events['t'],
                 x=events['x'],
                 y=events['y'],
        ...
        with ThreadPoolExecutor(max_workers=min(multiprocessing.cpu_count(), configure.max_threads_number_for_datasets_preprocess)) as tpe:
            ...
            for class_name in os.listdir(extract_root):
                ...
                for bin_file in os.listdir(aedat_dir):
                    ...
                    sub_threads.append(tpe.submit(ActionRecognition.read_aedat_save_to_np, source_file,
                               target_file))
